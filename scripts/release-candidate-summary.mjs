...
9. After restart/wake, use the trusted row reconnect \`Copy\` button to paste the full reconnect output: this computer, trusted device, \`Auto reconnect: enabled\`, \`Check: reachable\`, measured latency, last-seen timing, endpoint source, endpoint, last failure, and recovery evidence.
...
11. Use the Input Transport row \`Copy\` button on the Mac sender to paste \`Input Transport: Outgoing\`, \`This computer\`, \`Summary: key press r\`, target device, relative time, and \`Status: Accepted\` evidence for the accepted test input. Also use the Input Transport row \`Copy\` button on the Windows receiver to paste \`Input Transport: Incoming\`, \`This computer\`, \`Summary: key press r\`, source device, relative time, and \`Status: Accepted\` evidence.
12. Before pressing Stop, use the active capture \`Copy\` button to paste \`Capture: active\`, \`This computer\`, target, and started/elapsed-time evidence.
13. After pressing Stop, use the stopped capture \`Copy\` button to paste \`Capture: stopped\`, \`This computer\`, target, started/stopped timing, and \`Result: stopped cleanly\`; also paste receiver Input Transport row \`Copy\` output for accepted incoming mouse move, mouse click, scroll, and key events on the Windows receiver.
14. In the auto-discovery run, the copied reconnect evidence must show endpoint source as discovery, reconnect, or saved endpoint exactly as shown in the UI, and it must include measured latency.
15. In the manual fallback run, paste the peer \`This computer\` row's local endpoint \`Evidence\` output so the report records \`Local endpoint\`, \`This computer\`, \`Label: Best LAN IPv4\` / \`LAN IPv4\` / \`LAN IPv6\`, endpoint, TCP 44777, and private-network state; \`Endpoint used\` must match the copied endpoint.
16. In the manual fallback run, the copied reconnect evidence must show endpoint source as saved endpoint or manual IP and include measured latency, and the report must include a successful TCP 44777 probe such as Test-NetConnection, nc/netcat, telnet, socket connect, or port probe. If a retry was needed, use the trusted IP update \`Copy\` button to paste this computer, device, recovery action, endpoint field, current endpoint/source, last failure, and recovery hint evidence.

...
5. Record firewall evidence that shows allowed/successful rules for TCP \`44777\` and UDP \`44778\` on both computers.
6. Use the pending pairing row \`Copy\` button to paste pairing direction, this computer, visible code, typed-code state, local/remote approval, and expiry evidence into the pair-action, visible-code, typed-code, and pairing-evidence rows.
7. Use the Trusted Device Audit peer row \`Evidence\` button to paste this computer, local fingerprint, peer, peer role, and peer fingerprint evidence into the trusted and fingerprint rows.
8. Record a Test date that is on or after the release manifest date and not in the future.
9. Use the Startup health \`Copy\` button to paste \`Startup health\`, \`This computer\`, \`TCP:\`, \`UDP:\`, \`Start:\`, and started/reconnect timing evidence after restart.
10. After restart/wake, use the trusted row reconnect \`Copy\` button to paste the full reconnect output: this computer, trusted device, \`Auto reconnect: enabled\`, \`Check: reachable\`, measured latency, last-seen timing, endpoint source, endpoint, last failure, and recovery evidence.
11. Use the trusted row receive \`Copy\` button to paste \`Allow incoming control: enabled\`, \`Device receive: enabled\`, and \`Input control: ready\` evidence before input.
12. Use the Input Transport row \`Copy\` button on the Mac sender to paste \`Input Transport: Outgoing\`, \`This computer\`, \`Summary: key press r\`, target device, relative time, and \`Status: Accepted\` evidence for the accepted test input. Also use the Input Transport row \`Copy\` button on the Windows receiver to paste \`Input Transport: Incoming\`, \`This computer\`, \`Summary: key press r\`, source device, relative time, and \`Status: Accepted\` evidence.
13. Before pressing Stop, use the active capture \`Copy\` button to paste \`Capture: active\`, \`This computer\`, target, and started/elapsed-time evidence.
14. After pressing Stop, use the stopped capture \`Copy\` button to paste \`Capture: stopped\`, \`This computer\`, target, started/stopped timing, and \`Result: stopped cleanly\`; also paste receiver Input Transport row \`Copy\` output for accepted incoming mouse move, mouse click, scroll, and key events on the Windows receiver.
15. In the auto-discovery run, the copied reconnect evidence must show endpoint source as discovery, reconnect, or saved endpoint exactly as shown in the UI, and it must include measured latency.
16. In the manual fallback run, paste the peer \`This computer\` row's local endpoint \`Evidence\` output so the report records \`Local endpoint\`, \`This computer\`, \`Label: Best LAN IPv4\` / \`LAN IPv4\` / \`LAN IPv6\`, endpoint, TCP 44777, and private-network state; \`Endpoint used\` must match the copied endpoint.
17. In the manual fallback run, the copied reconnect evidence must show endpoint source as saved endpoint or manual IP and include measured latency, and the report must include a successful TCP 44777 probe such as Test-NetConnection, nc/netcat, telnet, socket connect, or port probe. If a retry was needed, use the trusted IP update \`Copy\` button to paste this computer, device, recovery action, endpoint field, current endpoint/source, last failure, and recovery hint evidence.
18. Confirm Notes show \`Blocking issues: none\`, identify screenshots or logs captured for pairing, reconnect, input, and capture evidence, and show \`Retest required: no\`; unresolved blocking issues, incomplete captured-evidence notes, or required retests fail release readiness.
19. Verify the completed report:

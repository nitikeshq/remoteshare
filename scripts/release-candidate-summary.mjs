...
11. After restart/wake, use the trusted row reconnect \`Copy\` button to paste the full reconnect output: this computer, trusted device, \`Auto reconnect: enabled\`, \`Check: reachable\`, measured latency, last-seen timing, endpoint source, endpoint, last failure, and recovery evidence.
...
17. In the manual fallback run, paste the peer \`This computer\` row's local endpoint \`Evidence\` output so the report records \`Local endpoint\`, \`This computer\`, \`Label: Best LAN IPv4\` / \`LAN IPv4\` / \`LAN IPv6\`, endpoint, TCP 44777, and private-network state; \`Endpoint used\` must match the copied endpoint.

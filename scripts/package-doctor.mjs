...
check("Incoming input auth and identity failures record trusted source failure", networkRuntime.includes("record_failed_input_source") && networkRuntime.includes("authenticated_input_failure_records_source_failure") && networkRuntime.includes("authenticated_input_failure_ignores_invalid_source_control_port") && networkRuntime.includes("incoming_input_identity_failure_records_source_failure_without_input_audit") && networkRuntime.includes("if !store.trusted_identity_matches(&source)") && networkingMilestone.includes("Incoming input authentication and trusted-identity failures from a trusted device ID are also recorded as trusted-device failures") && networkingMilestone.includes("Authenticated identity failures are rejected before an input transport row is recorded"));

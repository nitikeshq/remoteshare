...
check("Runtime applies persisted state changes after successful saves", runtimeStore.includes("settings_save_failure_does_not_mutate_runtime_settings") && runtimeStore.includes("manual_endpoint_save_failure_does_not_update_runtime_status") && runtimeStore.includes("receive_shortcut_save_failure_does_not_toggle_runtime_permissions") && runtimeStore.includes("device_receive_save_failure_does_not_toggle_runtime_permission") && runtimeStore.includes("forget_trusted_device_save_failure_keeps_runtime_state") && runtimeStore.includes("state.persisted = persisted") && architecture.includes("swapped into live runtime memory only after the local save succeeds") && architecture.includes("failed saves from creating in-memory trust, endpoint, or permission state"));

...
check("Scan LAN replies only to accepted peers", runtimeStore.includes("pub fn record_peer(&self, announcement: PeerAnnouncement, endpoint: String) -> bool") && runtimeStore.includes("return false;") && /true\s+}/.test(runtimeStore) && networkRuntime.includes("let accepted = store.record_peer") && networkRuntime.includes("accepted && reply_requested") && networkingMilestone.includes("same self-device, normalized endpoint, private-network guard, trusted fingerprint, and advertised public-key validation"));

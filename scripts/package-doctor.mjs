...
check("Scan LAN replies only to accepted peers", runtimeStore.includes("pub fn record_peer(&self, announcement: PeerAnnouncement, endpoint: String) -> bool") && runtimeStore.includes("return false;") && /true\s+}/.test(runtimeStore) && networkRuntime.includes("let accepted = store.record_peer") && networkRuntime.includes("accepted && reply_requested") && networkingMilestone.includes("same self-device, trusted fingerprint, and advertised public-key validation"));
...
check("Networking milestone documents role capabilities", networkingMilestone.includes("local role: `Main`, `Client`, or `Both`") && networkingMilestone.includes("`Main` and `Both` can send") && networkingMilestone.includes("`Client` and `Both` can receive") && networkingMilestone.includes("trusted-device records carry the peer role"));

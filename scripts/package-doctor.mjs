...
check("Release workflow creates tagged releases as drafts", workflow.includes("draft: true") && readme.includes("creates a draft GitHub Release") && releaseChecklist.includes("creates a draft GitHub Release") && releaseChecklist.includes("Keep the release as a draft until `npm run verify:release-readiness"));
...
check("README documents non-empty release installer verification", readme.includes("expected non-empty installer type"));
check("README documents release manifest verification", readme.includes("RELEASE-MANIFEST.json") && readme.includes("verifies the manifest against the installers and combined checksum file"));
check("README documents manual assembled release asset", readme.includes("Manual workflow runs also assemble a single `remoteshare-release-assets` artifact") && readme.includes("prefilled `lan-smoke-report.md`") && readme.includes("release-candidate-summary.md"));
check("README documents release candidate job summary", readme.includes("publish `release-candidate-summary.md` into the GitHub Actions job summary") && readme.includes("read the release candidate details in the GitHub Actions job summary"));
check("README documents pre-tag native artifact promotion", readme.includes("For a pre-tag build") && readme.includes("download `remoteshare-release-assets`") && readme.includes("open `release-candidate-summary.md`") && readme.includes("GitHub artifact subdirectories under `release-artifacts`") && readme.includes("npm run verify:publish-artifacts -- release-artifacts") && readme.includes("npm run prepare:release-assets -- release-artifacts release-assets") && readme.includes("npm run verify:release-manifest -- release-assets") && readme.includes("npm run prepare:lan-smoke-report -- release-assets release-assets/lan-smoke-report.md") && readme.includes("npm run release:candidate-summary -- release-assets") && readme.includes("npm run release:smoke-rows -- release-assets"));
check("README documents final release readiness verifier", readme.includes("npm run prepare:lan-smoke-report") && readme.includes("npm run release:smoke-rows") && readme.includes("npm run verify:release-readiness") && readme.includes("completed LAN smoke report") && readme.includes("bundled prefilled `lan-smoke-report.md`") && readme.includes("release-candidate-summary.md` still match the release assets"));
check("README leads with early Mac-to-Windows LAN MVP", readme.includes("early LAN MVP") && readme.includes("Mac keyboard and mouse") && readme.includes("Windows computer"));
check("README documents X25519 pairing milestone", readme.includes("X25519-derived trusted control secrets"));
check("README documents typed code confirmation", readme.includes("Type the six-digit code shown on the other computer"));
check("README documents private-network inbound guard", readme.includes("private-network-only by default"));
check("README documents first MVP input direction", readme.includes("macOS as the sender/main computer") && readme.includes("Windows as the receiver/client"));
check("README documents security status", readme.includes("## Security Status") && readme.includes("trusted private LANs") && readme.includes("Full encrypted transport is still planned"));
check("README links release checklist", readme.includes("docs/release-checklist.md"));
check("README documents first LAN test flow", readme.includes("## First LAN Test"));
check("README first LAN test mentions reconnect setting", readme.includes("Confirm `Auto reconnect` is on") && readme.includes("Use `Check`"));
check("README links first LAN runbook", readme.includes("docs/first-lan-test.md"));
check("README links first LAN evidence report", readme.includes("docs/lan-smoke-report-template.md") && readme.includes("MVP acceptance evidence report"));
...
check("README documents manual endpoint copy fallback", readme.includes("copy this computer's endpoint"));
check("README documents IPv4 and IPv6 control listener support", readme.includes("IPv6 plus IPv4 listener support") && readme.includes("IPv4 and IPv6 endpoints"));
...
check("First LAN runbook documents local-only endpoint rejection", firstLanTest.includes("Manual pair rejects `localhost`") && firstLanTest.includes("localhost.localdomain") && firstLanTest.includes("0.0.0.0"));
...
check("LAN smoke report template documents manual endpoint evidence", lanSmokeReport.includes("peer computer's copied control endpoint") && lanSmokeReport.includes("discovery was disabled, skipped, unavailable, or failed") && lanSmokeReport.includes("endpoint was copied from the peer computer's `This computer` row") && lanSmokeReport.includes("192.168.1.20:44777") && lanSmokeReport.includes("[fd00::20]:44777"));
...
check("LAN smoke report preparation helper rejects mismatched release manifest versions", lanSmokePreparer.includes("manifest.version !== packageJson.version") && lanSmokePreparer.includes("artifact filename must include package version") && fs.readFileSync("scripts/test-prepare-lan-smoke-report.mjs", "utf8").includes("wrong manifest version should fail") && fs.readFileSync("scripts/test-prepare-lan-smoke-report.mjs", "utf8").includes("wrong artifact version should fail"));
...
check("Scan LAN replies only to accepted peers", runtimeStore.includes("pub fn record_peer(&self, announcement: PeerAnnouncement, endpoint: String) -> bool") && runtimeStore.includes("return false;") && runtimeStore.includes("true\n    }") && networkRuntime.includes("let accepted = store.record_peer") && networkRuntime.includes("accepted && reply_requested") && networkingMilestone.includes("same self-device, trusted fingerprint, and advertised public-key validation"));
check("UI keeps Manual pair control for runbook", appUi.includes("Manual pair"));
check("UI explains manual endpoint defaults and rejected local endpoints", appUi.includes("manual-endpoint-hint") && appUi.includes("Missing ports use 44777") && appUi.includes("Localhost, loopback, unspecified, and link-local IPv6 endpoints are rejected"));
...
check("UI requires typed pairing code before confirm", appUi.includes("pairingCodeEntries") && appUi.includes("Other code") && appUi.includes("pairingCodeEntryState") && appUi.includes("Code mismatch") && appUi.includes("disabled={!entryState.canConfirm}") && networkingMilestone.includes("Confirm` disabled until the typed six-digit code"));
...
check("Architecture documents authenticated trusted messages", architecture.includes("HMAC-SHA256 authentication") && architecture.includes("current ping challenge"));
...
check("Runtime exposes atomic receive shortcut", runtimeStore.includes("enable_receive_for_trusted_devices") && runtimeStore.includes("Receive enabled for {trusted_count} trusted device") && runtimeStore.includes("receive_shortcut_enables_global_and_trusted_device_permissions") && tauriAppRuntime.includes("enable_receive_for_trusted_devices"));
check("Runtime binds pong to ping challenge", networkRuntime.includes("challenge: response_challenge") && networkRuntime.includes("response_challenge == challenge"));
...
check("Runtime ignores late trusted failures for forgotten devices", runtimeStore.includes("trusted_connection_failure_ignores_unknown_device_without_ghost_failure") && runtimeStore.includes(".any(|device| device.id == device_id)") && networkingMilestone.includes("late reconnect/input failures for a device that has already been forgotten are ignored"));
...
check("Runtime binds explicit known-device endpoint pairing to selected identity", runtimeStore.includes("expected_pairing_peer_for_device") && runtimeStore.includes("pairing_peer_from_trusted_device") && runtimeStore.includes("explicit_endpoint_pairing_target_keeps_known_device_identity") && runtimeStore.includes("manual_endpoint_pairing_target_without_known_device_learns_identity_from_ack") && networkRuntime.includes("expected_peer.public_key != peer.public_key") && networkingMilestone.includes("explicit known-device endpoint pairings must match the selected device ID and fingerprint"));
...
check("Runtime validates trusted endpoint recording", runtimeStore.includes("trusted_connection_recording_normalizes_and_rejects_invalid_endpoints") && runtimeStore.includes("record_trusted_connection") && runtimeStore.includes("normalized_endpoint(&endpoint)"));
...
check("State storage hardens Unix config permissions", fs.readFileSync("src-tauri/src/identity.rs", "utf8").includes("0o700") && fs.readFileSync("src-tauri/src/identity.rs", "utf8").includes("0o600"));
...
check("UI exposes failed endpoint edit recovery", appUi.includes("Edit IP") && appUi.includes("Set IP") && appUi.includes("Verify IP") && appUi.includes("update_trusted_endpoint") && appUi.includes("canEditTrustedEndpoint") && appUi.includes("Copy the current endpoint from the other computer"));
...
check("Release manifest verifier enforces package-version manifest and filenames", releaseManifestVerifier.includes("manifest.version !== packageVersion") && releaseManifestVerifier.includes("must include package version") && releaseManifestVerifierTestIncludes("wrong manifest version should fail") && releaseManifestVerifierTestIncludes("wrong artifact filename version should fail"));
...
check("Release manifest verifier rejects mismatches, stale checksums, and empty artifacts", releaseManifestVerifier.includes("Manifest hash mismatch") && releaseManifestVerifier.includes("stale entry") && releaseManifestVerifier.includes("sizeBytes <= 0"));
...
check("Release summary reports publish readiness", releaseSummary.includes("Publish readiness") && releaseSummary.includes("exactly one installer per platform is present, non-empty, checksummed, and version-matched") && releaseSummary.includes("duplicatePlatforms") && fs.readFileSync("scripts/test-release-summary.mjs", "utf8").includes("Publish readiness: incomplete") && fs.readFileSync("scripts/test-release-summary.mjs", "utf8").includes("macOS DMG has 2 artifacts") && fs.readFileSync("scripts/test-release-summary.mjs", "utf8").includes("Publish readiness: exactly one installer"));

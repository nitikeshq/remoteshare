...
check("Incoming input auth failures record trusted source failure", networkRuntime.includes("record_failed_input_source") && networkRuntime.includes("authenticated_input_failure_records_source_failure") && networkRuntime.includes("authenticated_input_failure_ignores_invalid_source_control_port") && networkingMilestone.includes("Incoming input authentication failures from a trusted device ID are also recorded as trusted-device failures"));

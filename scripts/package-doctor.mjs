...
check("Runtime status prunes expired pending pairings", runtimeStore.includes("status_prunes_expired_pending_pairing_rows") && runtimeStore.includes("prune_runtime_state(&mut state, now_ms())") && networkingMilestone.includes("Pending requests expire after 120 seconds and are pruned from runtime state, including status snapshots used by the UI"));

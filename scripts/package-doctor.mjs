...
check("UI bases failure recovery hints on failed endpoint source", appUi.includes("failure.endpointSource === \"saved\"") && appUi.includes("failure.endpointSource === \"discovery\"") && appUi.includes("failure.endpointSource === \"health\"") && appUi.includes("Saved endpoint may be stale") && appUi.includes("Last reconnect endpoint may be stale") && runtimeStore.includes("struct ConnectionFailure") && runtimeStore.includes("endpoint_source: EndpointSource") && runtimeStore.includes("failure_endpoint_source(") && runtimeStore.includes("endpoint_source: failure.endpoint_source.clone()") && runtimeStore.includes("trusted_connection_failure_keeps_original_endpoint_source") && runtimeStore.includes("matches!(failure.endpoint_source, super::EndpointSource::Saved)") && runtimeStore.includes("failure.endpoint_source") && runtimeStore.includes("super::EndpointSource::Discovery"));
...
check("UI labels manual endpoint source as manual IP", appUi.includes('if (device.endpointSource === "manual") return "manual IP"'));
...
check("Runtime rolls back remote approval when pairing completion fails", runtimeStore.includes("pairing.remote_approved = false") && runtimeStore.includes("completed_pairing_rejects_public_trusted_endpoint_while_guard_is_on") && runtimeStore.includes("!pairing.remote_approved"));
...
check("Runtime normalizes trusted failure endpoints for status evidence", runtimeStore.includes("trusted_connection_failure_normalizes_endpoint_for_status") && runtimeStore.includes("normalized_endpoint(endpoint).unwrap_or_else") && networkingMilestone.includes("Failed endpoint values are normalized before they are shown in status"));
...
check("Accepted incoming input refreshes trusted source health", networkRuntime.includes("record_successful_input_source") && networkRuntime.includes("accepted_incoming_input_refreshes_source_health") && networkingMilestone.includes("Accepted incoming input also refreshes the receiver's trusted connection health"));
check("Incoming input auth failures record trusted source failure", networkRuntime.includes("record_failed_input_source") && networkRuntime.includes("authenticated_input_failure_records_source_failure") && networkingMilestone.includes("Incoming input authentication failures from a trusted device ID are also recorded as trusted-device failures"));
check("Incoming reconnect auth failures record trusted source failure", networkRuntime.includes("record_failed_reconnect_source") && networkRuntime.includes("authenticated_reconnect_failure_records_source_failure") && networkingMilestone.includes("Incoming reconnect ping authentication failures and trusted identity mismatches are also recorded as trusted-device failures"));
...
check("Capture target resolution clears stale active capture", runtimeStore.includes("active_capture_target_clears_capture_without_usable_endpoint") && runtimeStore.includes("clear_capture_state(&mut state)") && networkingMilestone.includes("active target no longer has any usable endpoint after runtime cleanup"));

...
8. Use the pending pairing row `Copy` button to capture direction, endpoint, visible code, typed-code state, local/remote approval, and expiry evidence.
9. Type the six-digit code shown on the other computer, then click `Confirm` on both computers.
10. Verify that the device row shows `Trusted` and a shortened key fingerprint.
11. Open `Trusted Device Audit` and use its `Key` buttons to copy or compare the full local and peer fingerprints for evidence.

...
| Failure reason before retry | `none` or visible UI diagnostic/recovery hint | `none` or trusted IP update `Copy` output with this computer, recovery action, endpoint field, copied endpoint, TCP port, current endpoint/source, last failure, and recovery hint |

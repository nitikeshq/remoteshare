...
- Device-row and pending-pairing actions are guarded by in-flight keys, so a tester cannot accidentally send duplicate Pair, Confirm, Check, Test, Capture, Stop, Forget, or Receive commands while waiting for one command result. This keeps first-LAN smoke evidence tied to one visible operation at a time.

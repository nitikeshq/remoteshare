...
- The UI can select one trusted device as the active host capture target, shows the active capture target plus elapsed start time, can copy that active capture evidence as `Capture: active`, `Target`, and `Started` smoke-report fields before Stop, and can stop capture mode once the native capture engine is ready.

...
- Incoming input authentication and trusted-identity failures from a trusted device ID are also recorded as trusted-device failures with the sender's current control endpoint, so the receiver's diagnostics show the auth or identity failure instead of only the input transport row. Authenticated identity failures are rejected before an input transport row is recorded.

...
- Accepted incoming input also refreshes the receiver's trusted connection health from the sender's current control endpoint, so the receiver has a verified reconnect candidate after manual input smoke succeeds.
- Incoming input authentication failures from a trusted device ID are also recorded as trusted-device failures with the sender's current control endpoint, so the receiver's diagnostics show the auth failure instead of only the input transport row.

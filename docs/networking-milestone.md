...
- Device rows can copy trusted reconnect smoke evidence as one string, including the local computer, Auto reconnect state, trusted device, Check result, last-seen timing, endpoint source, endpoint, input-control readiness, last failure, and recovery hint.
...
- Receivers log incoming input transport events with accepted/rejected state, event summary, source device, and relative time so smoke-test evidence can connect the `key press r` and capture events to the trusted sender. Each input transport row can copy those fields as one evidence string, including the failure reason when an event is rejected or fails.
...
- The UI can select one trusted device as the active host capture target, shows the active capture target plus elapsed start time, can copy that active capture evidence, and can stop capture mode once the native capture engine is ready.

...
- The UI keeps `Confirm` disabled until the typed six-digit code from the other computer matches the locally visible comparison code, labels six-digit mismatches before a backend request is sent, clears typed codes when a pending pairing disappears or is recreated with a new code/expiry, and keeps expired pending pairing rows visibly disabled while the local countdown waits for the next runtime status prune.

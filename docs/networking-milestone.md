...
- Device rows can copy trusted reconnect smoke evidence as one string, including the local computer, Auto reconnect state, trusted device, Check result, measured latency, last-seen timing, endpoint source, endpoint, input-control readiness, last failure, and recovery hint.

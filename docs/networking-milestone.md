...
- Windows injection is implemented with `SendInput` for mouse move, primary/secondary button down/up events, wheel scroll, letters, digits, common punctuation captured from macOS, modifier keys, arrow/navigation keys, and basic control keys including distinct Backspace and Delete. It still needs validation on a Windows runner and a real Windows desktop.

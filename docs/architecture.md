...
- On Unix-like systems, the config directory is written with `0700` permissions and `state.json` is written with `0600` permissions. State saves write and sync a temporary file, rename it into place, remove the temporary file if replacement fails, then sync the parent directory on Unix so identity and trusted-secret updates are less likely to be lost after a crash. OS keychain storage is still planned before broader public testing.
- Runtime changes that affect persisted settings, start-at-login status, manual endpoints, receive permissions, forgotten devices, trusted endpoints, or completed pairing trust are built in a cloned persisted state and swapped into live runtime memory only after the local save succeeds. This keeps failed saves from creating in-memory trust, endpoint, permission, or ready startup-registration state that disappears after restart. If the platform start-at-login registration succeeds but the settings save fails, RemoteShare rolls that OS registration back to the previous saved value and reports failed startup health instead of leaving persisted settings and OS launch behavior out of sync.
...
- If the persisted state file cannot be parsed or read, the failed state is copied to a timestamped backup before a fresh identity is created, so trusted-device recovery remains possible during testing. Backup names add a numeric suffix instead of overwriting an existing failed-state copy from the same second. Failed-state backups are also forced to private `0600` permissions on Unix because they can contain identity keys and trusted shared secrets.

...
The prepared `RELEASE-MANIFEST.json` records when release assets were generated. The completed LAN smoke report `Test date` must be an ISO `YYYY-MM-DD` date on or after that manifest date, so smoke evidence from an older candidate cannot be reused for newer assets.
